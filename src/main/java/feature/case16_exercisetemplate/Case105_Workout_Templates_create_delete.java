...
public class Case105_Workout_Templates_create_delete extends BasePage {
	private static final Logger LOGGER = LoggerFactory.getLogger(Case105_Workout_Templates_create_delete.class);
	private ElementActions elementActions;
	private WorkoutStepsHelper workoutStepsHelper;

    public Case105_Workout_Templates_create_delete(WebDriver driver) {
        super(driver);
    ...
    @Test
    public void createworkoutdelete() throws InterruptedException {
        ...
        try {
            ...
            // Click on submit button
            WebElement deleteicon =wait.until(ExpectedConditions.elementToBeClickable(weblocators.exercisedeleteicon));
            // Scroll to the "Staff Notes" element using JavaScriptExecutor
            ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView();", deleteicon);
            Thread.sleep(3500);
            elementActions.clickElement(weblocators.exercisedeleteicon);
            // Log message to console and Allure report
            LOGGER.info("Click on exercisedeleteicon");
            AllureUtils.logStep("Click on exercisedeleteicon");
      
            
            // Click on submit button
            WebElement confirmicon =wait.until(ExpectedConditions.elementToBeClickable(weblocators.confirmbttn));
            // Scroll to the "Staff Notes" element using JavaScriptExecutor
            ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView();", confirmicon);
            elementActions.clickElement(weblocators.confirmbttn);
            // Log message to console and Allure report
            LOGGER.info("Click on confirmbttn");
            AllureUtils.logStep("Click on confirmbttn");
