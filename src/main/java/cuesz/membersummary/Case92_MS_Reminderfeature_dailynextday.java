...
import cuesz.utils.reminderCalendarUtils;
import cuesz.utils.webTestdata;
import cuesz.utils.weblocators;
import io.qameta.allure.Allure;

public class Case92_MS_Reminderfeature_dailynextday extends BasePage {
	private static final Logger LOGGER = LoggerFactory.getLogger(Case92_MS_Reminderfeature_dailynextday.class);
  ...
  weblocators locators = new weblocators();
    ...
    public Case92_MS_Reminderfeature_dailynextday(WebDriver driver) {
        super(driver);
        elementActions = new ElementActions(driver); // Initialize ElementActions object    
    }

    @Test
    public void remindernextday() throws InterruptedException {
        Thread.sleep(2000);
		...
		int numOfIterations = 1; // Number of times to click the element
...
////        AllureUtils.captureScreenshot(driver, "reminder17");
        ...
        elementActions.clickElement(weblocators.calendarnextbttn);
        // Specify the date you want to select
        String dateToSelect = "May 01, 2024"; // Change this to the desired date

        // Use the utility method to select the date
        reminderCalendarUtils.selectDate(driver, dateToSelect);
